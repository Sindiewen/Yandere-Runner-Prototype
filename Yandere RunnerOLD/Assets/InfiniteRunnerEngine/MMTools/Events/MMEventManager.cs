...
{	
	...
	public static class MMEventManager 
	{
	    ...
	    public static void RemoveListener<MMEvent>( MMEventListener<MMEvent> listener ) where MMEvent : struct
	    {
	        ...
	        {
				#if EVENTROUTER_THROWEXCEPTIONS
					throw new ArgumentException( string.Format( "Removing listener, but the event type \"{0}\" isn't registered.", eventType.ToString() ) );
				#else
					return;
				#endif
	        }

	        var list = _subscribersList[eventType];
